...
//	private boolean mTestDelayElapsed; // add a timer for testing
	...
	public void onCreate() {
		...
		//不新生成activity
		notifIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK| Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);//关键的一步，设置启动模式
...
//		mNotif = Compatibility.createNotification(this, mNotificationTitle, "", R.drawable.status_level, IC_LEVEL_OFFLINE, bm, mNotifContentIntent, true);
	...
	private synchronized void setIncallIcon(IncallIconState state) {
		...
		bm=BitmapFactory.decodeResource(getResources(), R.drawable.boda);
	...
	private Class<? extends Activity> incomingReceivedActivity = LoginAppActivity.class;
...
//			text = String.format(text, id);
			text = String.format(text, SettingInfo.getAccount());
		}
...
//		mNotif = Compatibility.createNotification(this, mNotificationTitle, text, R.drawable.status_level, level, bm, mNotifContentIntent, true); 
		mNotif = Compatibility.createNotification(this, mNotificationTitle, text, R.drawable.icon, level, bm, mNotifContentIntent, true);
	...
	public void deleteOldAccount() {
		...
		for (int i = 0; i < nbAccounts; i++) {
			...
			if (mPrefs.getAccountDomain(accountId) == null
					|| "null".equals(BaseUntil.stringNoNull(mPrefs
							.getAccountDomain(accountId)))) {
				break;
			}
			try {
...
//		}
	...
	public void onCallStateChanged(final LinphoneCall call, final State state, final String message) {
		...
		if (state == LinphoneCall.State.IncomingReceived) {
			incomingReceivedActivity=IncomingCallActivity.class;
			onIncomingReceived();
		}
