...
public class NetImageLoader {
	public interface OnFinishListener {
		void onFinish(int result, Bitmap b);
	}

	private OnFinishListener mFinishedlistener;

	private final String DEAFULT_PATH = YETApplication.getRenhuaSdcardDir();
	 ...
	 */
	public NetImageLoader(RequestQueue queue, ImageCache imageCache) {
		mRequestQueue = queue;
		mCache = imageCache;
	}
	 ...
	 */
	@SuppressLint("NewApi")
	public static ImageListener getImageListener(final ImageView view, final int defaultImageResId, final int errorImageResId, final OnFinishListener finishedListener) {
		return new ImageListener() {
			...
			public void onResponse(ImageContainer response, boolean isImmediate) {
				if (response.getBitmap() != null) {
					if (view != null) {
						BitmapDrawable bitmapDrawable = new BitmapDrawable(YETApplication.getContext().getResources(), response.getBitmap());
						view.setBackground(bitmapDrawable);
//						view.setImageBitmap(response.getBitmap());
					}
					if (finishedListener != null) {
						finishedListener.onFinish(0, response.getBitmap());
					}
				} else if (defaultImageResId != 0) {
					if (view != null)
						view.setBackgroundResource(defaultImageResId);
					if (finishedListener != null)
						finishedListener.onFinish(-1, null);
				}
			}
		};
	}
	 ...
	 */
	public ImageContainer get(final String requestUrl, ImageListener imageListener, int maxWidth, int maxHeight) {
		...
		if (cachedBitmap != null) {
			...
			Log.v("NetImageLoader", "found bitmap from cache by key:" + cacheKey);
			return container;
		}

		File file = new File(DEAFULT_PATH + "/" + urlToFilePath(requestUrl));
		if (file.exists()) {
			Bitmap sdcardBitmap = BitmapFactory.decodeFile(DEAFULT_PATH + "/" + urlToFilePath(requestUrl));
			if (sdcardBitmap != null) {
				ImageContainer container = new ImageContainer(sdcardBitmap, requestUrl, null, null);
				imageListener.onResponse(container, true);
				Log.v("NetImageLoader", "found bitmap from file system by file name:" + DEAFULT_PATH + "/" + urlToFilePath(requestUrl));
				mCache.putBitmap(cacheKey, sdcardBitmap);
				return container;
			}
		}
		...
		Request<?> newRequest = new ImageRequest(requestUrl, new Listener<Bitmap>() {
			...
			public void onResponse(Bitmap response) {
				onGetImageSuccess(cacheKey, response, requestUrl);
			}
	 ...
	 */
	private void onGetImageSuccess(String cacheKey, Bitmap response, String requestUrl) {
		// cache the image that was fetched.
		mCache.putBitmap(cacheKey, response);
		File myCaptureFile = new File(DEAFULT_PATH + "/" + urlToFilePath(requestUrl));
	...
	private String urlToFilePath(String url) {
		...
		if (i > 0 && i < sdpath.length()) {
			sdpath = sdpath.substring(0, i);
			sdpath += ".ren";
		}
		return sdpath;
	}
