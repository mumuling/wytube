...
import com.cqxb.yecall.t9search.model.Contacts.SearchByType;
...
public class ContactsHelper {
	...
	public void setmBaseContacts(List<Contacts> mBaseContacts) {
		this.mBaseContacts = mBaseContacts;
	}
		...
		for (int i = 0; i < contactsCount; i++) {
			...
			if (true == T9MatchPinyinUnits.matchPinyinUnits(pinyinUnits, name,search, chineseKeyWord)) {// search by NamePinyinUnits;
				...
				for(currentContacts=mBaseContacts.get(i),firstContacts=currentContacts; null!=currentContacts; currentContacts=currentContacts.getNextContacts()){
					...
					if(currentContacts.getPhoneNumber().length()<7){
						currentContacts.setSipaccount(currentContacts.getPhoneNumber());
					}
					currentContacts.setTelephoneNumber(currentContacts.getPhoneNumber());
					mSearchByNameContacts.add(currentContacts);
					currentContacts.setContactName(name);
					currentContacts.setNumber(currentContacts.getPhoneNumber());
				}
	...
	private void initContactsHelper() {
		mContext = YETApplication.getContextObject();
	...
	private void parseContacts(List<Contacts> contacts) {
		...
		for (Contacts contact : contacts) {
			if (!mBaseContacts.contains(contact)) {
				Log.e("", "mBaseContacts.contains="+contact.getPhoneNumber());
