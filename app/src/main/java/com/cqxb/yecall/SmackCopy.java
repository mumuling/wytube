...
import com.cqxb.offline.OfflineMessageManager;
...
//	private String YETIP="euc.yuneasy.cn";
	...
	public static SmackCopy instance;
    ...
    private ConnectionListener mcConnectionListener;
    private Timer tExit;
	 ...
	 * @return
	 */
	public static synchronized SmackCopy getInstance(){
		if(instance==null){
			instance=new SmackCopy();
		}
		return instance;
	}
...
//			config.setSendPresence(false); // where connConfig is object of .  
...
//			}).start();
			...
			mcConnectionListener=new ConnectionListener() {
...
//		        	tExit.schedule(new timetask(), logintime);
			...
			//监听连接
			conn.addConnectionListener(mcConnectionListener);
...
//						System.out.println(" registerChatListener:   "+msg.getBody());
 					// 收到的消息
                      ...
                      //转换消息格式
        				...
        				if(!dbHelper.updateData(InformationList.TABLE, initialValues, InformationList.FRIENDID+" = ?", new String[]{fromJID})){
        					dbHelper.insertData(InformationList.TABLE, null, initialValues);
        				}
        				dbHelper.close();
        				Intent intent=new Intent(action);
               ...
               try {
					if (packet instanceof Presence) {
						...
						if("307".equals(code)){//请出房间
							...
							dbHelper.deleteData(GroupChatEntity.TABLE, GroupChatEntity.ROOMID+" = ? ", new String[]{roomid});
							dbHelper.deleteData(InformationList.TABLE, InformationList.ROOMID+" = ? ", new String[]{roomid});
							Intent intent=new Intent(action);
						...
						if (presence.getType().equals(Presence.Type.subscribe)) {
							...
							if (vcard != null) {
							 ...
							 if(!dbHelper.updateData(InformationList.TABLE, initialValues, InformationList.FRIENDID+" = ? and "+InformationList.FLAG +" = ?", new String[]{jid,"1"})){
								 dbHelper.insertData(InformationList.TABLE, null,initialValues);
							 }
							 Intent intent=new Intent(action);
							...
							String jid = getJabberID(presence.getFrom());
							boolean deleteData = dbHelper.deleteData(InformationList.TABLE, InformationList.FRIENDID+" = ? ", new String[]{jid});
							boolean deleteData2 = dbHelper.deleteData(ContactEntity.TABLE, ContactEntity.FRIENDID+" = ?", new String[]{jid});
							boolean deleteData3 = dbHelper.deleteData(SingleChatEntity.TABLE, SingleChatEntity.FRIENDID+" = ?", new String[]{jid});
...
//				MultiUserChat multiUserChat = new MultiUserChat(null, null);
			...
			public void invitationReceived(Connection arg0, String room, String inviter,
				...
				Cursor data = dbHelper.getData("select _id from "+InformationList.TABLE+" where "+InformationList.ROOMID+" = ? and "+InformationList.FLAG+" = ? ", new String[]{room,"4"});
				if(data!=null){
					if(data.getCount()>0){
						return;
					}
				...
				if(!dbHelper.updateData(InformationList.TABLE, initialValues, InformationList.ROOMID+" = ? ", new String[]{room})){
					dbHelper.insertData(InformationList.TABLE, null, initialValues);
				}
	...
	public void removeListener(){
		if(conn==null){
			...
			try {
				...
				conn.getChatManager().removeChatListener(mChatManagerListener);
				MultiUserChat.removeInvitationListener(conn,invitationListener);
				conn.removeConnectionListener(mcConnectionListener);
	...
	public boolean addUser(String userName, String name, String groupName) {
		...
		if(userName.indexOf("@")==-1){
			userName=userName+"@"+SmackCopy.conn.getServiceName();
		}
	...
	class timetask extends TimerTask {
		...
		public void run() {
			if (SettingInfo.getAccount() != null&& SettingInfo.getPassword() != null) {
				...
				// 连接服务器
				if (login(SettingInfo.getParams(PreferenceBean.USERIMACCOUNT, ""), SettingInfo.getParams(PreferenceBean.USERIMPWD, ""),SettingInfo.getParams(PreferenceBean.CLIENT, "android"))) {
	...
	private void off(){
		IQ iq=new IQ() {
			...
			public String getChildElementXML() {
				return "<query xmlns=\"http://jabber.org/protocol/disco#items\" node=\"http://jabber.org/protocol/offline\"/>";
			}
		};
		iq.setType(Type.GET);
		iq.setTo("montague.net");
		conn.sendPacket(iq);
		System.out.println("carbon:"+iq.toXML());
	}
	
	
	private void offInfo(){
		try {  
			System.out.println("离线消息数量: .................." );  
            ...
            {  
                ...
                for (int i = 0; i < ms.size(); i++) {  
                	System.out.println("离线消息："+ms.get(i).getBody());
                }  
            }  
